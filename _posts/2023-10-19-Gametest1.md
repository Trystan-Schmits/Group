    ...
    document.addEventListener("keydown", function (e) {
        if (e.key === "e" || e.key === "E") {
            if (checkForOverlap(myCharacterObject, boxObject1)) {
                // If the "E" key is pressed and there's an overlap with boxObject1, make it disappear
                boxObject1.visible = false;
            }
        }
    });

    function checkForOverlapWithEKey() {
        if (checkForOverlap(myCharacterObject, EKeyObject)) {
            console.log("Now press the E key");
            animateEKey();
        }

        if (boxObject1.visible) {
            display.draw(1);
        }
...
//create objects
    ...
    //Ekey
        ...
        var EKeyObject = new Object("Ekey",EKeyImage,[20,16],[100,80],[500,500],1,1);   
    // Animation code for the EKey
    var eKeyAnimationFrames = 2; 
    var eKeyAnimationFrameWidth = 20; // Width of each frame
    var eKeyAnimationFrameHeight = 16; // Height of each frame
    var eKeyAnimationFrameIndex = 0;
    var eKeyAnimationFrameDelay = Math.round(fps / 8); // Adjust the delay for the animation speed

    function animateEKey() {
    EKeyObject.srcX = eKeyAnimationFrameIndex * eKeyAnimationFrameWidth;
    eKeyAnimationFrameIndex = (eKeyAnimationFrameIndex + 1) % eKeyAnimationFrames;
    setTimeout(animateEKey, eKeyAnimationFrameDelay);
    }
...
function frame(){ //when a frame is updated
    ...
    if (checkForOverlap(myCharacterObject, boxObject1)) {
        console.log("Now press the E key");
         EKeyObject.OverridePosition([20, 16], [100, 90], [500, 500], 1, 1);
    }
    if (checkForOverlap(myCharacterObject, boxObject2)) {
        console.log("Now press the E key")
    }

    // Display the EKey object initially
    EKeyObject.OverridePosition([20, 16], [100, 80], [500, 500], 1, 1);
