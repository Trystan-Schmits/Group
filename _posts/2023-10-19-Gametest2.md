...
function checkCollision() {
    if (checkForOverlap(myCharacterObject, boxObject1) || checkForOverlap(myCharacterObject, boxObject2)) {
        // Character is near the boxes, display a "P" button
        document.getElementById("button").style.display = "block";
    } else {
        // Character is not near the boxes, hide the button
        document.getElementById("button").style.display = "none";
    }
}

var button = document.createElement("button");
button.innerHTML = "Press 'P'";
button.style.display = "none"; // Initially hidden
button.id = "button";

// Add an event listener to respond to the "P" button press
button.addEventListener("click", function() {
    // Perform an action when the "P" button is pressed
    alert("You pressed 'P'");
});
