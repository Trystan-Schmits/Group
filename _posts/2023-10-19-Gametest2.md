...
messageElement.innerHTML = "Press the letter P";
messageElement.style.position = "absolute";
messageElement.style.top = "10px"; // Adjust the position as needed
messageElement.style.left = "10px"; // Adjust the position as needed
...
window.addEventListener('keydown', function(e) { //prevent space from moving screen
  if(e.keyCode == 32 && e.target == document.body) {
    ...
    if (checkCollision(myCharacterObject, doorObject)) {
        messageElement.style.visibility = "visible"; // Show the message when colliding with the door
    } else {
        messageElement.style.visibility = "hidden"; // Hide the message when not colliding
    }
frame();
</script>